...
def run_script(script_name, description):
    ...
    # (ใช้ Path เต็มๆ ทุกจุด)
    command = f'"{CONFIG["python_exe"]}" "{script_path}" "{CONFIG["data_path"]}"'
    try:
        # ใช้ subprocess.run เพื่อแสดงผลลัพธ์ (Logs)
        subprocess.run(
            command,
            shell=True,
            check=True
...
def main():
    ...
    # 2. ยืนยันก่อนเริ่มเทรน
    print("\n WARNING: This will overwrite your existing models in the 'models/' folder.")
    ...
    if not run_script("train_sentiment.py", "STEP 1: Train Sentiment Classification Model"):
        print("\n X PIPELINE HALTED: Sentiment training failed. Cannot proceed to Category model.")
        sys.exit(1)

    if not run_script("train_categories.py", "STEP 2: Train Category Classification Model"):
        print("\n X PIPELINE HALTED: Category training failed.")
        sys.exit(1)
    ...
    if missing:
        print("\n WARNING: Some model files are missing. Check the logs above for errors.")
    else:
        print("\n ready to run the API: uvicorn main:app --reload --port 8000")
